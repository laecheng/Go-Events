	...
	public Set<Item> getFavoriteItems(String userId) {
		...
		for (String itemId : itemIds) {
			...
			if (iterable.first() != null) {
				...
				builder.itemId(doc.getString("item_id"));
				builder.name(doc.getString("name"));
				builder.address(doc.getString("address"));
				builder.url(doc.getString("url"));
				builder.imageUrl(doc.getString("image_url"));
				builder.rating(doc.getDouble("rating"));
				builder.distance(doc.getDouble("distance"));
	...
	public void saveItem(Item item) {
		...
		if (iterable.first() == null) {
			db.getCollection("items").insertOne(new Document()
					.append("item_id", item.getItemId()).append("distance", item.getDistance())
